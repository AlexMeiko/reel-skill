...
6. 数字/术语先搜再写，不要用训练记忆编造。只披露季度就按季度，搜不到写「未披露」。
7. 写台词稿（能念的句子，按镜头/意群，不要一条一个标语）。冻结后才 TTS。
8. 有配音：TTS → `ffprobe` 量时长 → 排该段 `timeline.json` → 生成 `part-NN.srt` → 再画。禁止按 4–5 字/秒估时先画再套 TTS。
...
| 下一段开头认得出上一段主角 | 承接不是半透明残影叠字，旧层要淡到看不见 |
| 教学数字有标注 | 权重/分数写「示意」，不要装成论文数据 |
